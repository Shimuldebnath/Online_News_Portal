...
<?php
  ...
  $query = "SELECT * FROM international_news ORDER BY id desc";
  $result=mysqli_query($con,$query);

  while($row=mysqli_fetch_array($result)){

    $headline = $row['headline'];
    $content = substr($row['news'],0,500);
    $image = $row['image'];
    $post_id= $row['id'];
    ...
    <a href="newinternational.php?p_id=<?php echo "$post_id"?>"id="Read">Read More...</a>
